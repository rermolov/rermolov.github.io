...
function GetCacheBaseName() {
    return GAME_NAME + CACHE_NAME_PREFIX + self.registration.scope
}
...
async function GetMainPageUrl() {
    ...
    for (const t of e) {
        let e = t.url;
        if (e.startsWith(self.registration.scope) && (e = e.substring(self.registration.scope.length)),
        e && "/" !== e)
...
function fetchWithBypass(e, t) {
    ...
    t) {
        ...
        return t.search += Math.floor(1e6 * Math.random()),
        fetch(t, {
            headers: e.headers,
            mode: e.mode,
...
async function CreateCacheFromFileList(e, t, a) {
    ...
    for (const e of n)
        e.ok || (o = !1,
        console.error(CONSOLE_PREFIX + "Error fetching '" + e.url + "' (" + e.status + " " + e.statusText + ")"));
    if (!o)
    ...
    try {
        ...
        throw console.error(CONSOLE_PREFIX + "Error writing cache entries: ", t),
        caches.delete(e),
        t
    }
}
async function UpdateCheck(e) {
    try {
        ...
        if (!t.ok)
            throw new Error(OFFLINE_DATA_FILE + " responded with " + t.status + " " + t.statusText);
        const a = await t.json()
          , n = a.version
          , o = a.fileList
          , r = a.lazyLoad
          , s = GetCacheVersionName(n);
        if (await caches.has(s))
            return void (await IsUpdatePending() ? (console.log(CONSOLE_PREFIX + "Update pending"),
            Broadcast("update-pending")) : (console.log(CONSOLE_PREFIX + "Up to date"),
            Broadcast("up-to-date")));
        const i = await GetMainPageUrl();
        o.includes("index.html") || o.unshift("./"),
        i && -1 === o.indexOf(i) && o.unshift(i),
        console.log(CONSOLE_PREFIX + "Caching " + o.length + " files for offline use"),
        e ? Broadcast("downloading") : BroadcastDownloadingUpdate(n),
        r && await WriteLazyLoadListToStorage(r),
        await CreateCacheFromFileList(s,o,!e),
        await IsUpdatePending() ? (console.log(CONSOLE_PREFIX + "All resources saved, update ready"),
        BroadcastUpdateReady(n)) : (console.log(CONSOLE_PREFIX + "All resources saved, offline support ready"),
...
async function GetCacheNameToUse(e, t) {
    ...
    return console.log(CONSOLE_PREFIX + "Updating to new version"),
...
self.addEventListener("fetch", e=>{
    ...
    const t = "navigate" === e.request.mode
      , a = HandleFetch(e, t);
    t && e.waitUntil(a.then(()=>UpdateCheck(!1))),
    e.respondWith(a)
}
);
